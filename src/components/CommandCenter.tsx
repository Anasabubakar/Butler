...
import { CalendarEvent, GmailMessage, Task, Note } from "../types";
      ...
      <div className="px-8 pb-10">
        <div className="grid grid-cols-12 gap-5">
              ...
              {(events.length ? events : DEMO_AGENDA).slice(0, 6).map((ev, i) => (
                <div key={ev.id || i} className="grid grid-cols-[60px_1fr_auto] gap-3 items-baseline">
                  ...
                  <span className="body-sm truncate max-w-[180px]" style={{ color: "var(--color-b-text-tertiary)" }}>
                    {(ev as any).note || (ev.location ? "· " + ev.location : "held")}
                  </span>
                </div>
              ))}
          ...
          <Card tone="raised" className="col-span-12 xl:col-span-3 p-6 min-h-[320px]">
            ...
            <div className="flex flex-col gap-2.5">
              {(emails.length ? emails : DEMO_MAILS).slice(0, 5).map((m: any, i) => (
          ...
          <Card tone="raised" className="col-span-12 xl:col-span-4 p-6 min-h-[260px]">
            ...
            <div className="flex flex-col gap-2.5">
              {(notes.length ? notes : DEMO_NOTES).slice(0, 4).map((n, i) => (
                <div key={(n as any).id || i} className="flex items-start justify-between gap-3">
                  <span className="body-sm flex-1" style={{ color: "var(--color-b-text-primary)" }}>· {(n as any).title || (n as any).content?.slice(0, 60)}</span>
                  <span className="mono-sm flex-shrink-0" style={{ color: "var(--color-b-text-tertiary)" }}>{formatAgo((n as any).updatedAt)}</span>
                </div>
              ))}
            </div>
            <button
...
const DEMO_AGENDA: CalendarEvent[] = [
  { id: "1",  summary: "Standup",         start: "09:00", end: "09:30", note: "no doc · quiet" } as any,
  { id: "2",  summary: "Meridian HQ",     start: "10:45", end: "11:30", note: "moved by Butler" } as any,
  { id: "3",  summary: "Lunch break",     start: "12:30", end: "13:30", note: "held" } as any,
  { id: "4",  summary: "Deep work",       start: "13:30", end: "16:00", note: "protected" } as any,
  { id: "5",  summary: "1:1 with Kai",    start: "16:00", end: "16:45", note: "draft topics ready" } as any,
  { id: "6",  summary: "Board pack review", start: "18:00", end: "18:45", note: "62% ready" } as any,
];
const DEMO_MAILS = [
...
const DEMO_DELEGATED = [
  ...
  { svc: "Notion", ctx: "Q3 rituals",      title: "New page: I've drafted the outline." },
];
const DEMO_NOTES = [
  { id: "d1", title: "Kai prefers dry replies before 10am.",     updatedAt: new Date(Date.now() - 86400000 * 1).toISOString() },
  { id: "d2", title: "Meridian is sensitive to deployment windows.", updatedAt: new Date(Date.now() - 86400000 * 2).toISOString() },
  { id: "d3", title: "Board reads best with a graph up front.",  updatedAt: new Date(Date.now() - 86400000 * 5).toISOString() },
  { id: "d4", title: "Late-week Fridays: no deep-work holds.",   updatedAt: new Date(Date.now() - 86400000 * 8).toISOString() },
] as any;
const GITHUB_EVENTS = [
